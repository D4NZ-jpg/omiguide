...
export function GuideShell({
    ...
    React.useEffect(() => {
        try {
            const v = localStorage.getItem("omi:sidebar-collapsed");
    ...
    function toggleCollapsed() {
        setCollapsed((prev) => {
            const next = !prev;
            try {
                localStorage.setItem("omi:sidebar-collapsed", next ? "1" : "0");
            } catch {
                // ignore
            }
            return next;
            ...
            >
                <div className="w-full max-w-3xl">
                    ...
                    <p className="mt-4 text-xs text-muted-foreground">
                        Edita archivos en{" "}
                        <code className="rounded bg-muted px-1">content/guide</code> para actualizar
                        el sitio.
                    </p>
                </div>
            </main>
